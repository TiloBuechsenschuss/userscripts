...
| `charpane-heal.js` | charpane | The two sidebar buttons. **heal** casts your heal skills until HP is full or none can raise it further; **max** sits next to each prolongable buff you can actually cast and re-casts it as many times as your MP allows, measuring the per-cast cost live so gear and effect discounts are accounted for. A "refresh skills" button re-reads the skills page after you learn or lose one |
...
| `equip-optimize.js` | inventory | Two inventory tools. **Optimize for this** equips the highest-value item per slot for whatever the enchantment-sort dropdown is sorting by, with element / Monster Level / encounter pickers for those sorts. **Collapse all / Expand all** flips every inventory category open or closed at once |
